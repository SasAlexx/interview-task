...
def year_perfomance(year: str, db: Session):
    ...
    for month in months:
        ...
        credits_plans = db.query(Plan).filter(Plan.period == sql_month).filter(Plan.category_id == 3).all()
        payments_plans = db.query(Plan).filter(Plan.period == sql_month).filter(Plan.category_id == 4).all()
        plan_info = PlanInfo()
        plan_info.month_year = month[3:]

        for plan in credits_plans:
            if plan.category_id == 3:
                plan_info.credits_month_plan_sum = db.query(Plan.sum).filter(Plan.period == sql_month).filter(
                    Plan.category_id == 3).scalar()

                plan_credits = db.query(Credit).filter(Credit.issuance_date.ilike(f'{sql_month.year}-{sql_month.month}%')).all()
        ...
        for plan in payments_plans:
            if plan.category_id == 4:
                plan_info.payments_month_plan_sum = db.query(Plan.sum).filter(Plan.period == sql_month).filter(
                    Plan.category_id == 4).scalar()

                plan_payments = db.query(Payment).filter(Payment.payment_date.ilike(f'{sql_month.year}-{sql_month.month}%')).all()
